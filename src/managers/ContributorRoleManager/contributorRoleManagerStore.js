...
export const useContributorRoleManagerStore = defineComponentStore(
	...
	(props) => {
		...
		// Fetch add/edit form
		const {apiUrl: contributorRoleFormApiUrl} = useUrl(
			'contributorRoles/contributorRoleFormComponent',
		);
		const {data: contributorRoleForm, fetch: fetchContributorRoleForm} =
			useFetch(contributorRoleFormApiUrl, {
				method: 'GET',
			});
		fetchContributorRoleForm();
		 ...
		 */
		function getContributorRoleForm(action, role, roleId = null) {
			...
			if (roleId && action === Actions.EDIT) {
				...
				setMethod('PUT');
				setAction(`${preparedForm.action}/${roleId}`);
			}
		 ...
		 * @param {Int} roleId
		 */
		function roleDelete(role, roleId) {
			const {openDialog} = useModal();
			const {apiUrl} = useUrl(`contributorRoles/${roleId}`);
				...
				bodyProps: {
					onConfirm: async () => {
						...
						if (isSuccess.value) {
							...
							await fetchContributorRoleForm();
						}
					},
		 ...
		 * @param {Int} roleId
		 */
		function roleEdit(role, roleId) {
			openContributorRoleFormModal(
				t('manager.contributorRoles.edit'),
				getContributorRoleForm(Actions.EDIT, role, roleId),
			);
		}
